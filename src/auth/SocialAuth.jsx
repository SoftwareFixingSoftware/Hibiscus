...
export default function SocialAuth({ disabled = false, type = 'signin', onError = () => {} }) {
  ...
  const handleGithubLogin = () => {
    ...
    // GitHub OAuth parameters - you need to get these from your GitHub OAuth App
    const clientId = "Ov23liBQxrfZlrAxa7HN"; // TODO: Replace with your actual GitHub Client ID
  ...
  const handleGoogleLogin = () => {
    ...
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        // If One Tap isn't displayed, render button
  ...
  return (
    <div className="social-buttons">
      ...
      <motion.button 
        ...
        className={`btn-social btn-github ${disabled ? 'btn-disabled' : ''}`}
        disabled={disabled}
      >
      ...
      <div id="googleButton" className="google-button-wrapper">
        <motion.button 
          type="button" 
          onClick={handleGoogleLogin}
          whileHover={!disabled ? { scale: 1.02 } : {}}
          whileTap={!disabled ? { scale: 0.98 } : {}}
          className={`btn-social btn-google ${disabled ? 'btn-disabled' : ''}`}
          disabled={disabled}
        >
